...
void SingleFrameQuantizeDequantize(char *fileName,char *name, int *XY,int quantizationLevel)
{
...
int *SendMCUs(char *fileName, int quantizationLevel, int *dimensions)
{
	int XY[2] = {0};
	byte *buffer = OpenFile(fileName, XY);
	int dataLength = XY[0] * XY[1];
...
void ReceiveMCUs(int *coefficients, char *fileName, char *fileNameNew, int *XY, int quantizationLevel)
{
	int dataLength = XY[0] * XY[1];
	int numberOfMCUs = dataLength / 64;
...
size_t IntArrayToCharArray(int *source, char *destination, int intLength)
{
	char stringBuffer[6];
	size_t start = 0;
	for(int i = 0; i < intLength; i++)
	{
		snprintf(stringBuffer,6, "%d ", source[i]);
		for(size_t j = 0; j < strlen(stringBuffer);j++)
		{
			destination[j+start] = stringBuffer[j];
		}
		start += strlen(stringBuffer);
	}
	/*for(size_t i = 0 ; i < start; i++)
	{
		printf("%c", destination[i]);
	}*/
	destination[start] = '\0'; //strtok requires null terminated string
	return start+1;
}

char *LZ4Encoder(char *data,size_t length,int *compressedDataSize)
{
	int sourceSize = (int)(length);
	int maxDistanceSize = LZ4_compressBound(sourceSize);
	char *compressedData = malloc((size_t)maxDistanceSize);
	*compressedDataSize = LZ4_compress_default(data, compressedData, sourceSize, maxDistanceSize);
	//printf("Now : %d, Ratio: %.2f\n",*compressedDataSize,(float) *compressedDataSize/sourceSize);	
	return compressedData;
}

char *LZ4Decoder(char *compressedData,size_t originalDataSize,int compressedDataSize, int *decompressedDataSize)
{
	int sourceSize = (int)(originalDataSize);
	char *decompressedData = malloc(sourceSize);
	*decompressedDataSize = LZ4_decompress_safe(compressedData,decompressedData,compressedDataSize,sourceSize);
	//printf("Now : %d\n",*decompressedDataSize);
	return decompressedData;	
}

void CharArrayToIntArray(char *source,int *destination,int *length)
{
	char *integer = strtok(source," ");
	int count = 0;
	int i = 0;
	while(integer != NULL)
	{
		sscanf(integer,"%d",&count);
		destination[i] = count;
		integer = strtok(NULL," ");
		i++;
	}
...
void SingleFrameSendReceive(char *fileName, char *saveFile,char *saveFile2,int quantizationLevel)
{
	...
	int *toSend = SendMCUs(fileName,quantizationLevel,dimensions);
	char *toCompress = malloc(dimensions[0]*dimensions[1]*6);
	size_t charArrayLength = IntArrayToCharArray(toSend, toCompress,dimensions[0]*dimensions[1]);
	char *compressedData = LZ4Encoder(toCompress, charArrayLength,&compressedDataSize);
	//Send int compressedDataSize,size_t charArrayLength, int dimensions[2], char* compressedData
	
	
	int decompressedDataSize = 0;
	char *decompressedData = LZ4Decoder(compressedData,charArrayLength,compressedDataSize,&decompressedDataSize);
	int  *decompressedInts = malloc(dimensions[0]*dimensions[1]*sizeof(int));
	int length2 = 0;
	CharArrayToIntArray(decompressedData,decompressedInts, &length2);
	ReceiveMCUs(toSend,saveFile,saveFile2,dimensions,quantizationLevel);
	free(toSend);	
}

//Returns compressed char array
void PackageAndSendFrame(char *fileName)
{
	int dimensions[2];
	int compressedDataSize = 0;
	size_t charArrayLength = 0;
	int quantizationLevel = 1;
	int *dataNotReady = SendMCUs(fileName,quantizationLevel,dimensions);
	char *toCompress = malloc(dimensions[0]*dimensions[1]*6);//Six assumes largest int is -9999 or 99999
	charArrayLength = IntArrayToCharArray(dataNotReady, toCompress,dimensions[0]*dimensions[1]);
	char *compressedData = LZ4Encoder(toCompress, charArrayLength,&compressedDataSize);
	
	//Write to file
	FILE *fp = fopen("bicho.bae", "wb");
	fwrite(&quantizationLevel,4,1,fp);
	fwrite(&compressedDataSize,4,1,fp);
	fwrite(&charArrayLength,8,1,fp);
	fwrite(dimensions,8,1,fp);
...
int UnpackageAndReceiveFrame(char *fileName, char *pgmName, char *receivedFileLocation)
{
	...
	FILE *fp = fopen(receivedFileLocation, "rb");
	fread(&quantizationLevel,4,1,fp);
	fread(&compressedDataSize,4,1,fp);
	fread(&charArrayLength,8,1,fp);
	fread(dimensions,8,1,fp);
	
	char *compressedData = malloc(compressedDataSize);
	fread(compressedData,compressedDataSize,1,fp);
	fclose(fp);
	...
	int length2 = 0;
	CharArrayToIntArray(decompressedData,decompressedInts, &length2);
	//Have to save pgm for SingleFrameQuantizeDequantize(name,newFile,XY, quantizationLevel2);
	ReceiveMCUs(decompressedInts,fileName,pgmName,dimensions,quantizationLevel);

	free(compressedData);
	free(decompressedData);
	free(decompressedInts);
	return 1;
}
...
int main()
{
	char *fileName = "frame-6.pgm";
	char *saveFile = "sample2.bmp";
	char *initialFile = "sample2.pgm";
	char *newFile = "baba.bmp";
	int quantizationLevel = 1;
	int quantizationLevel2 = 1;
	int XY[2] = {0,0};
	char *name = "frame-3.pgm";
	SingleFrameSendReceive(fileName,saveFile,initialFile,quantizationLevel);
	//SingleFrameQuantizeDequantize(name,newFile,XY, quantizationLevel2);
	//char *fileName = "frame-1.pgm";
	//PackageAndSendFrame(fileName);
	char *received = "a.bmp";
	char *pgmName = "a.pgm";
	UnpackageAndReceiveFrame(received,pgmName,receivedFileLocation);
	return 0;
}*/
